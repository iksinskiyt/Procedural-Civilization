public class Bakery extends Building {
    ...
    private void produceFood(Inventory inventory){
        if(inventory.useItem(Inventory.ItemType.MEAT, meatCost)){ // why does it use 5 meat to make 1 food? don't ask me, i have no idea, its 01:30AM and it's gotta be optimized somehow... maybe the meat from ladybugstore has a fuckton of water inside
            inventory.addItem(Inventory.ItemType.FOOD, 1); // why the hell does the bakery bake meat?????? what????????
        }
